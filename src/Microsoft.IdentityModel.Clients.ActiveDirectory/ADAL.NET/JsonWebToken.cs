...
namespace Sandboxable.Microsoft.IdentityModel.Clients.ActiveDirectory
{
    public class JsonWebToken
    {
        ...
        public ClientAssertion Sign(ClientAssertionCertificate credential)
        {
            ...
            var token = this.Encode(credential);

            // Length check before sign
            if (MaxTokenLength < token.Length)
            {
                throw new Exception(AdalError.EncodedTokenTooLong);
            }

            return new ClientAssertion(this._payload.Issuer, string.Concat(token, ".", UrlEncodeSegment(credential.Sign(token))));
        }
        ...
        {
            // Header segment
            var jsonHeader = EncodeHeaderToJson(credential);

            var encodedHeader = EncodeSegment(jsonHeader);

            // Payload segment
            var jsonPayload = this.EncodePayloadToJson();

            var encodedPayload = EncodeSegment(jsonPayload);
        ...
        {
            return EncodeToJson(this._payload);
        }
        ...
        public class JWTPayload
        {
            ...
            [DataMember(Name = JsonWebTokenConstants.ReservedClaims.Subject, IsRequired = false, EmitDefaultValue = false)]
        ...
        public sealed class JWTHeaderWithCertificate : JWTHeader
        {
            ...
            public string X509CertificateThumbprint
            {
                get
                {
                    ...
                    return Base64UrlEncoder.Encode(this.Credential.Certificate.PublicKey().GetCertHash());
                }
